...
@ActiveProfiles("test")
class CorsAndSwaggerHealthTest {
    ...
    @Test
    @DisplayName("Swagger OpenAPI 3.0 명세 검증: /v3/api-docs 호출 시 정상 OpenAPI 스펙 반환")
    void testSwaggerApiDocsEndpoint() throws Exception {
        mockMvc.perform(get("/v3/api-docs"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.openapi").exists())
                .andExpect(jsonPath("$.info.title").value("Doro IAM API Documentation"));
    }

    @Test
    @DisplayName("Spring Boot Actuator 헬스체크 검증: /actuator/health 호출 시 200 OK")
